...
public partial class RotationConfigWindow : Window
{
	...
	[
		...
		"使用左上角搜索框可直接跳转到设置项。",
		"点击搜索结果中的外链图标可跳转到对应菜单。",
		"右键点击设置标签可复制即用的 /rotation Settings 命令。",
		"Actions 标签页：点击技能图标可配置、启用/禁用或设置快捷键。",
		"Actions：切换'在冷却窗口显示'可将技能加入冷却覆盖层。",
		"Actions：启用'拦截'可让 RSR 触发你排队的技能（仅 PvE）。",
		"UI > Information：启用 DTR 状态、Toast 提示、原始冷却和这些提示。",
		"UI > Windows：启用下一技能、控制、冷却和时间轴窗口。",
		"下一技能：'无输入'和'不移动'选项可改变覆盖层行为。",
		"仅在副本中或有敌人时显示窗口：UI > Windows > 仅在有敌人或副本中显示。",
		"List 标签页：管理驱散、优先状态、击退、无敌和止息列表。",
		"List 标签页：使用'重置并更新'可快速恢复精选列表。",
		"状态列表：按'+'可按名称或 ID 搜索；支持模糊搜索。",
		"状态列表：右键点击图标可移除；弹窗中也支持 Delete 键。",
		"Target 标签页：调整目标选择、视野锥形、接敌行为和木桩/Boss 处理。",
		"Target 标签页：设置 /rotation Cycle 行为和目标选择延迟。",
		"通过聊天管理 TargetingTypes：/rotation Settings TargetingTypes add|remove <类型>。",
		"Auto > Action Usage：允许/禁用 oGCD、设置 AoE 模式、爆发药、打断和正北。",
		"Auto > Healing：调整治疗阈值和非治疗职业的治疗行为。",
		"治疗：在 Auto > Healing 中自定义复活/Swiftcast 和优先级。",
		"地面 AoE：Auto > Healing 中有智能放置有益地面技能的选项。",
		"Basic > Timer：调整提前量和最小更新时间以平衡性能与穿插。",
		"Basic > Auto Switch：倒计时、死亡、区域切换等自动开关。",
		"教学模式会高亮目标；颜色设置在 UI > Information。",
		"Job 标签页：编辑 DNC 舞伴、SGE Kardia 坦克和 AST 卡片优先级。",
		"About > Macros 列出可用的聊天/宏命令和有用语法。",
		"About > Links：打开配置文件夹、GitHub、Ko-fi 和 Discord。",
		"Extra > Internal：安全地备份/恢复配置。",
		"Extra：可选调整，如移除动画/冷却延迟。",
		"点击侧边栏左下角的立方体图标可将诊断信息复制到剪贴板。",
		"时间轴窗口可可视化近期技能（UI > Windows）。",
		"打出伤害，不要阵亡",
		"治疗：唯一重要的 HP 是最后一点",
		"与人为善",
		"需要时可用“/statusoff <名称>”移除部分自身增益（如疾跑）。",
		"RSR 在传统类型移动设置下效果最佳。"
	];
				...
				else if (item == RotationConfigWindowTab.DutyRotation && Player.Object != null)
				{
					...
					if (CNLanguageClient)
					{
						displayName = true switch
						{
							...
							var _ when DataCenter.Orbonne => "副本 - 瓯博讷修道院",
							_ => "副本",
						};
					}
					else
